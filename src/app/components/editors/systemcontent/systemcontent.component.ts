...
@Component({
  selector: 'app-systemcontent',
  templateUrl: './systemcontent.component.html',
  styleUrls: ['./systemcontent.component.css',
...
export class SystemcontentComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.lookupcmspagemaster = this.cmsService.selectCmsMsLookupSystem(this.globalVariable.getProject());
  ...
  async selectchangeid(changeid) {
    ...
    if (this.changeid != 'null' && this.pageselect != 'null') {
      const cmspagefile = await this.cmsService.selectCmsSystemFile(this.pageselect, this.changeid);
  ...
  async onSubmit(event: any) {
    ...
    if (this.checkrequiredata() === true) {
      ...
      this.cmsService.updCmsSystemFile(JSON.stringify(objData));
